...
# --- 2. מנוע סריקה משופר V25 ---
def extract_data_v25(image):
    # סריקה מקיפה של כל הטקסט
    ...
    for line in lines:
        for heb_word, eng_key in hebrew_mapping.items():
            if heb_word in line or eng_key in line.upper():
                # מציאה של כל המספרים בשורה (כולל נקודה עשרונית)
                nums = re.findall(r"(\d+\.?\d*)", line)
                if nums:
                    # לוגיקה: התוצאה היא בדרך כלל המספר הראשון או האחרון בשורה שאינו טווח
                    # כאן ניקח את המספר הראשון שזוהה כתוצאה פוטנציאלית
                    val = float(nums[-1] if len(nums) > 1 and eng_key in ["CREA", "ALT"] else nums[0])
                    results[eng_key] = val
    return results

# --- 3. UI ---
st.set_page_config(page_title="Foodels Omega V25", layout="wide")
...
dog_age = st.sidebar.number_input("גיל:", 0.1, 25.0, 1.0)
...
uploaded_file = st.file_uploader("העלה את צילום המסך של הבדיקה", type=["jpg", "png", "jpeg"])

if uploaded_file:
    img = Image.open(uploaded_file)
    data = extract_data_v25(img)
