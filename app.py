...
# --- 2. מאגר גנטי וגזעים מורחב (הארסנל המלא) ---
breed_intelligence = {
    ...
    "פיטבול": {"genetics": "אלרגיות עור ובעיות בבלוטת התריס.", "risk_markers": ["EOS", "CHOL"], "rec": "מזון היפו-אלרגני על בסיס דגים."},
...
# --- 4. מנוע הסריקה החכם (Deep Scan V19) ---
def extract_data_v19(image):
    ...
    for i in range(len(d['text'])):
        ...
        for heb_word, eng_key in hebrew_mapping.items():
            if heb_word in word or (len(word) > 2 and word.upper() in eng_key):
                ...
                if line_nums:
                    # לוגיקה חכמה: בטופס של בונו, התוצאה היא בדרך כלל המספר הראשון 
                    # שמופיע מיד אחרי שם המדד ולפני טווח הנורמה בסוגריים
                    results[eng_key] = line_nums[0]
    return results

# --- 5. ממשק המערכת (UI) ---
st.set_page_config(page_title="Foodels AI V19 - Deep Scan", layout="wide")
