...
st.set_page_config(page_title="Foodels Masterpiece V48", layout="wide")
st.markdown("""
    <style>
    ...
    div.stButton > button { width: 100%; background-color: #25D366; color: white; font-weight: bold; border-radius: 10px; height: 50px; }
    </style>
    """, unsafe_allow_html=True)

SHOP_INFO = "פודלס - נחום שריג 33, שכונת רמות, באר שבע | 08-6655443"

# --- 2. דאטה-בייס אנציקלופדי מלא (כל המדדים והפיצ'רים) ---
blood_db = {
    "GLU": {"name": "גלוקוז", "min": 74, "max": 143, "unit": "mg/dL", "desc": "בודק רמת סוכר בדם (סוכרת/סטרס).", "cause": "סוכרת או סטרס חריף."},
    ...
    "BUN/CREA": {"name": "יחס אוריאה/קריאטינין", "min": 10, "max": 25, "unit": "", "desc": "יחס להערכת מקור הבעיה הכלייתית.", "cause": "חוסר איזון תפקודי."},
    ...
    "ALB/GLOB": {"name": "יחס אלבומין/גלובולין", "min": 0.6, "max": 1.5, "unit": "", "desc": "עוזר לאבחן מחלות כרוניות.", "cause": "שינוי במאזן החלבונים."},
...
# --- 3. מנוע סריקה V48 (דיוק עשרוני + חילוץ מטא-דאטה) ---
def extract_v48(pdf_file):
    res = {"data": {}, "meta": {}}
                ...
                for key in blood_db.keys():
                    if key in line_u:
                        ...
                        if match:
                            ...
                            if key == "CREA" and val > 5.0: continue # מניעת בלבול עם Ratio
                            res["data"][key] = val
    return res
...
if uploaded_file:
    result = extract_v48(uploaded_file)
    ...
    if data:
        ...
        with col2:
            ...
            else: st.success("✅ המדדים שנבדקו תקינים ומאוזנים.")
        ...
        st.markdown(f'<a href="https://wa.me/?text={url}" target="_blank"><button>📲 שלח דו"ח וטסאפ מלא</button></a>', unsafe_allow_html=True)
