...
# --- 1. מפה דו-לשונית מורחבת (כל המדדים האפשריים) ---
hebrew_mapping = {
...
# --- 2. מאגר גנטי וגזעים מורחב (הארסנל המלא של פודלס) ---
breed_intelligence = {
    ...
    "פיטבול/אמסטף": {"genetics": "אלרגיות עור ובעיות בבלוטת התריס.", "risk_markers": ["EOS", "CHOL"], "rec": "מזון היפו-אלרגני על בסיס דגים."},
...
# --- 4. מנוע סריקה Grid Neural Scan (V21) ---
def extract_data_v21(image):
    d = pytesseract.image_to_data(image, config=r'--oem 3 --psm 11 -l heb+eng', output_type=pytesseract.Output.DICT)
    results = {}
    ...
    for i in range(len(d['text'])):
        ...
        for heb_word, eng_key in hebrew_mapping.items():
            if heb_word in word or (len(word) > 2 and word.upper() in eng_key):
                curr_y = d['top'][i]
                # סריקת רשת - חיפוש מספרים בכל הטווח האופקי של השורה
                for j in range(len(d['text'])):
                    if abs(d['top'][j] - curr_y) < 30: # רגישות גובה מוגברת
                        num_match = re.search(r"(\d+\.?\d*)", d['text'][j])
                        if num_match:
                            val = float(num_match.group(1))
                            # סינון חכם של מספרי טווח הנורמה
                            if eng_key not in results:
                                results[eng_key] = val
    return results

# --- 5. ממשק המערכת (UI) ---
st.set_page_config(page_title="Foodels AI V21 - Neural Vision", layout="wide")
...
st.title(f"🚀 פיענוח Neural Vision עבור {dog_name}")
...
uploaded_file = st.file_uploader("העלה את טופס הבדיקה לסריקה סופית", type=["jpg", "png", "jpeg"])

if uploaded_file:
    img = Image.open(uploaded_file)
    data = extract_data_v21(img)
    ...
    if data:
        ...
        with col2:
            ...
            if score < 85: st.warning("מומלץ ייעוץ תזונתי להתאמת מזון תומך בחנות.")
