...
# --- 2. מאגר גנטי וגזעים מלא (Elite Edition) ---
breed_intelligence = {
    ...
    "רועה גרמני": {"genetics": "דיספלסיה בירך ובעיות עיכול (EPI).", "risk_markers": ["AMYL", "TP"], "rec": "מזון Mobility ואנזימי עיכול."},
    ...
    "לברדור/גולדן": {"genetics": "מפרקי ירך, השמנה וגידולים.", "risk_markers": ["CHOL", "GLU"], "rec": "מזון דל קלוריות וסיוע למפרקים."},
    "בולדוג צרפתי": {"genetics": "בעיות נשימה (BOAS) ואלרגיות עור.", "risk_markers": ["WBC", "EOS"], "rec": "חלבון מפורק ומשקל מבוקר."}
}

blood_db_base = {
    "CREA": {"name": "קריאטינין (כליות)", "min": 0.5, "max": 1.5, "unit": "mg/dL", "cause": "עומס כלייתי, התייבשות או פגיעה בתפקוד."},
...
# --- 3. מנוע סריקה מרחבי (Geometry-Aware Scan) ---
def extract_data_v20(image):
    ...
    for i in range(len(d['text'])):
        ...
        for heb_word, eng_key in hebrew_mapping.items():
            if heb_word in word or (len(word) > 2 and word.upper() in eng_key):
                curr_y = d['top'][i]
                line_data = []
                # אוסף את כל המספרים והמיקומים שלהם בשורה
                for j in range(len(d['text'])):
                    if abs(d['top'][j] - curr_y) < 25:
                        num_match = re.search(r"(\d+\.?\d*)", d['text'][j])
                        if num_match:
                            line_data.append({'val': float(num_match.group(1)), 'x': d['left'][j]})
                
                # מיון המספרים משמאל לימין (בטופס עברי, התוצאה בדרך כלל שנייה או שלישית מימין)
                line_data.sort(key=lambda x: x['x'])
                if line_data:
                    # בטופס של בונו, המספר הראשון משמאל לשם המדד הוא התוצאה
                    results[eng_key] = line_data[0]['val']
    return results

# --- 4. UI ---
st.set_page_config(page_title="Foodels AI V20", layout="wide")
...
st.title(f"🚀 פיענוח Column-Aware ל{dog_name}")

if dog_breed in breed_intelligence:
    with st.expander("🧬 פרופיל גנטי", expanded=True):
...
if uploaded_file:
    ...
    data = extract_data_v20(img)
    
    if data:
        issues = 0
        wa_sum = f"סיכום בריאות ל*{dog_name}* מפודלס:\n"
            ...
            for m, val in data.items():
                if m in blood_db_base:
                    ...
                    with st.expander(f"{info['name']}: {val} {'🚨' if is_bad else '✅'}", expanded=is_bad):
                        ...
                        wa_sum += f"{'📍' if is_bad else '✅'} {info['name']}: {val}\n"
        with col2:
            score = max(0, 100 - (issues * 12))
            st.metric("Health Score", f"{score}%")
            st.progress(score / 100)

        msg = urllib.parse.quote(wa_sum)
        st.markdown(f'<a href="https://wa.me/?text={msg}" target="_blank"><button style="background-color: #25D366; color: white; border: none; padding: 15px; border-radius: 8px; width: 100%; cursor: pointer;">📲 שלח לווטסאפ</button></a>', unsafe_allow_html=True)
