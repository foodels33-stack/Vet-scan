...
blood_db_base = {
    ...
    "RBC": {"name": "כדוריות אדומות", "min": 5.5, "max": 8.5, "unit": "M/uL", "cause": "דימום, אנמיה או מחלת מח עצם."},
    "WBC": {"name": "כדוריות לבנות", "min": 6.0, "max": 17.0, "unit": "K/uL", "cause": "זיהום חיידקי או דלקת פעילה."},
    "CREA": {"name": "קריאטינין (כליות)", "min": 0.5, "max": 1.5, "unit": "mg/dL", "cause": "עומס כלייתי, התייבשות או פגיעה בתפקוד הכליות."},
    "BUN": {"name": "אוריאה", "min": 7.0, "max": 27.0, "unit": "mg/dL", "cause": "תפקוד כליות ירוד או תזונה עתירת חלבון."},
...
# --- 2. מנוע סריקה הוליסטי (V28) ---
def extract_data_ultimate(image):
                ...
                for j in range(len(d['text'])):
                    if abs(d['top'][j] - curr_y) < 35:
                        ...
                        if num_match:
                            ...
                            if 0.1 <= val <= 2000 and val != 6789123: # סינון רעשים
                                line_nums.append(val)
                if line_nums:
                    results[eng_key] = line_nums[-1] if len(line_nums) > 1 else line_nums[0]
    return results

# --- 3. ממשק המערכת ---
st.set_page_config(page_title="Foodels Ultimate AI V28", layout="wide")
...
if uploaded_file:
    ...
    data = extract_data_ultimate(img)
    if data:
        issues = 0
