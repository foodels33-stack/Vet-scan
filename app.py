...
# --- 1. מילון זיהוי מורחב (הוספת כל המדדים מהטופס של בונו) ---
hebrew_mapping = {
...
# --- 2. מאגר גזעים מורחב (כולל רועה גרמני, בלגי, מלינואה) ---
breed_intelligence = {
...
# --- 3. מנוע סריקה מרחבי משופר ---
def extract_data_v15(image):
    ...
    for i in range(len(d['text'])):
        ...
        for heb_word, eng_key in hebrew_mapping.items():
            if heb_word in word or (len(word) > 2 and word.upper() in eng_key):
                # מצאנו "עוגן" (שם המדד), עכשיו נחפש מספר קרוב באותה רמת גובה
                curr_y = d['top'][i]
                for j in range(max(0, i-10), min(len(d['text']), i+15)):
                    if abs(d['top'][j] - curr_y) < 25: # סריקת שורה רחבה
                        potential_num = d['text'][j].replace(',', '.')
                        num_match = re.search(r"(\d+\.\d+|\d+)", potential_num)
                        if num_match:
                            val = float(num_match.group(1))
                            # סינון מספרים שהם טווחי נורמה (כמו 0.5 או 1.5)
                            if eng_key not in results:
                                results[eng_key] = val
    return results

# --- 4. ממשק (UI) ---
st.set_page_config(page_title="Foodels Absolute AI V15", layout="wide")
